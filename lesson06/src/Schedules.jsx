...
// 보완할 점 : 할일을 추가할 때, 같은 날짜이면서 같은 시간이면
...
export default function Schedules() {
  ...
  const handleDateChange = (date) => {
    setCalendarDate(date) // date 는 new Date() 형식의 객체
  ...
  const handleAllDelete = () => {
    ...
    // ㄴ 로컬 상태 업데이트를 추가하기 : time 이 null 일때, schedules 상태값을 해당 날짜제외하고 필터링
    ...
    >
      ...
      <div style={{ display: 'flex' }}>
        ...
        <div style={{ marginBottom: '2rem', width: '400px' }}>
          ...
          {/*
          선택된 날짜에 selectedSchedule.todos 가 있을 때만 TodosCardList 컴포넌트를 만들어요
          selectedSchedule.todos 는 selectedSchedule 객체가 null 일때(false) 실행하면
